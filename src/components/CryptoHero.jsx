    ...
    <div className="relative h-dvh w-screen overflow-x-hidden bg-gradient-to-br from-slate-900 via-purple-900 to-slate-900">
      ...
      <div className="absolute inset-0 overflow-hidden">
        {[...Array(15)].map((_, i) => (
          <div
            key={i}
            className="crypto-bg-element absolute w-4 h-4 bg-yellow-400/20 rounded-full"
            style={{
        ...
        {[...Array(8)].map((_, i) => (
          <div
            key={`crypto-${i}`}
            className="floating-crypto absolute text-2xl text-yellow-400/30"
            style={{
              left: `${Math.random() * 100}%`,
              top: `${Math.random() * 100}%`,
            }}
      ...
      <div className="relative z-10 h-dvh w-screen flex items-center justify-center">
        ...
        <div className="absolute inset-0 flex items-center justify-center">
          <div className="crypto-main-symbol text-9xl text-yellow-400 crypto-glow">
            ◊
          </div>
        </div>
        ...
        <div className="relative z-20 w-full h-full flex flex-col justify-center items-center text-center px-5">
          <h1 className="special-font hero-heading text-6xl md:text-8xl text-white crypto-glow mb-6">
            Panch<b>C</b>oin
          </h1>
          ...
          <Button
            ...
            containerClass="!bg-yellow-400 !text-black flex-center gap-1 crypto-glow text-lg px-8 py-4"
          />
        </div>

        {/* Corner text */}
        <h1 className="special-font hero-heading absolute bottom-5 right-5 z-30 text-yellow-400 crypto-glow text-4xl md:text-6xl">
          Panch<b>C</b>oin
        </h1>
      </div>
    </div>
